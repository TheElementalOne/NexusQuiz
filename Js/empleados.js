class EmpleadoManager {
  constructor() {
    ...
    this.formEditar.addEventListener("submit", this.actualizarEmpleado.bind(this)
    );
  ...
  async cargarEmpleados() {
    ...
    document.querySelectorAll(".btn-editar").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        ...
        // Cuando se hace clic en editar, carga el empleado específico
        this.abrirModal(id);
      });
    });
  }
  ...
  async cargarAreas() {
    ...
    // Actualiza los selects de creación y edición
    const selectsArea = [
      this.selectArea,
    ...
    selectsArea.forEach((select) => {
      select.innerHTML = '<option value="">Seleccione un área</option>'; // Añadir un placeholder
      data.forEach((area) => {
  ...
  async abrirModal(empleadoId) {
    try {
      ...
      document.getElementById("editar_boletin_empleado").checked = data.BOLETIN == 1;
      document.getElementById("editar_descripcion_empleado").value = data.DESCRIPCION;
      document
      ...
      this.cerrarModal(); // Cerrar el modal si hay un error
    }
  }
  ...
  obtenerDatosFormulario(tipo) {
    ...
    const form =
      tipo === "crear"
        ? this.formCrear
        : document.getElementById("formEditarEmpleado"); // Obtener el formulario correcto para editar
    const formData = new FormData(form); // Usar FormData para simplificar

    const data = {
      id: formData.get("id"),
      nombre: formData.get("nombre"), // Los names en HTML no tienen prefijo
